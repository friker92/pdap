...
\sectionDark{Programación Funcional}

\begin{frame}
  \frametitle{Programación Funcional}
  \alert{Características:}
  \begin{itemize}
  \item Código inteligente.
  \item Menor complejidad.
  \item Mayor modularidad.
  \end{itemize}
  \alert{Herramientas:}
  \begin{itemize}
  \item Mutación.
...
\sectionDark{JavaScript en 4 minutos}

\begin{frame}[fragile,fragile]
  \frametitle{JS en 4 min}
  \alert{Expresiones:}
\begin{lstlisting}
...
\begin{frame}[fragile,fragile]
  \frametitle{JS Funcional}
  ``Lazy'' es una librería que nos permite realizar \textbf{evaluación
  perezosa}.
\begin{lstlisting}
...
\begin{itemize}
\item \textbf{chunk: } agrupa una secuencia en secuencias de tamaño n.
\item \textbf{compact:} quita de la secuencia \emph{false, 0, ``'',
    null} y \emph{undefined}.
\item \textbf{reduce}, \textbf{map}, \textbf{filter},
  \textbf{reverse}, \textbf{each}, \textbf{take}, ...
\end{itemize}
\begin{lstlisting}
// Primeros 10 cuadrados divisibles por 3
...
function f2(b){return b + ' 2';}
function f3(c){return c + ' 3';}

var composition = f3.compose(f2).compose(f1); // f3(f2(f1(___)))

console.log( composition('count') ); // -> 'count 1 2 3'
// f3(f2(f1('count'))) = f3(f2('count' + ' 1')) = 
//                     = f3('count 1' + ' 2') =
//                     = 'count 1 2' + ' 3' = 
//                     = 'count 1 2 3'
\end{lstlisting}
...
\begin{frame}
  ...
  \begin{enumerate}
  \item D. Mantyla, ``Functional programming in JavaScript''. Packt
    Publishing, 2015.
  \item M. Haverbeke,
    \href{http://eloquentjavascript.net/1st_edition/chapter6.html}{``Eloquent
      JavaScript'', A Modern Introduction to Programming} 
  \item ``Librería Lazy.js''\\\url{http://danieltao.com/lazy.js/}
  \item ``Librería Underscore.js''\\\url{http://underscorejs.org/}
  \item Luis Atencio, ``Functional Programming in Javascript''
    \\\url{https://dzone.com/storage/assets/379008-rc217-functionalprogramming.pdf}
  \item ``Javascript Garden''\\\url{http://bonsaiden.github.io/JavaScript-Garden}
